...
def run_analysis(request):
    ...
    artefacts_qs = Artefact.objects.filter(
        id__in=artefact_ids,
        assigned_to=request.user
    ).prefetch_related(
        "incident",
        Prefetch("records", queryset=LogRecord.objects.only("id", "record_index", "content"))
    )

    if not artefacts_qs.exists():
    ...
    # Materialize lists to avoid repeated DB hits
    artefacts = list(artefacts_qs)
    rules = list(rules_qs)

    def stream(artefacts, rules, request):
        total_artefacts = len(artefacts)
        total_rules = len(rules)
        total_matches = 0

        yield f'<p class="text-primary"><i class="fas fa-play-circle"></i> Starting rule-based analysis...</p>\n'
        yield f'<p class="text-info"><i class="fas fa-info-circle"></i> Analyzing {total_artefacts} artefact(s) with {total_rules} rule(s)</p>\n'
                ...
                for artefact_index, artefact in enumerate(artefacts, start=1):
                    ...
                    for record_index, record in enumerate(records, start=1):
                        ...
                        for rule_index, rule in enumerate(rules, start=1):
                            ...
                            if record_index == 1:
                                yield f'<p class="text-secondary"><i class="fas fa-cogs"></i> Applying Rule [{rule_index}/{total_rules}]: <strong>{rule.name}</strong></p>\n'
    ...
    return StreamingHttpResponse(stream(artefacts, rules, request), content_type='text/html')
...
def ai_intel_generated_rules_view(request):
    context = {"response": generate_rules_from_internet_intel()}
